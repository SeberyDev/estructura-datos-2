const burbuja = (arr) => {
  //A la constante burbuja le asignamos una funcion anonima.
  ...
  for (let i = 0; i < longitud; i++) {
    ...
    for (let j = 0; j < longitud - 1 - i; j++) {
      //Comenzamos un segundo siclo para realizar las comparaciones.
      if (arr[j] < arr[j + 1]) {
        //Si el elemento es mayor al de la derecha entonces.
    ...
    console.log(`${arr}`);
    if (ordenado) {
      //si ordenado es verdadero nos indica que no se realizaron cambios al arreglo
